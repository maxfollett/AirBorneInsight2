...
try:
    ...
    # Save the georeferenced bands and ratios
    transform = from_bounds(lon_min, lat_min, lon_max, lat_max, bands["B4"].shape[1], bands["B4"].shape[0])
    crs = "EPSG:4326"

    def save_geotiff(data, name):
        output_filename = os.path.join(output_folder, f"{current_date}_{survey_name}_{name}.tiff")
        with rasterio.open(output_filename, 'w', driver='GTiff', height=data.shape[0], width=data.shape[1],
    ...
    for band_name, band_data in bands.items():
        save_geotiff(band_data, band_name)

    # Save ratios (convert to float32 for compatibility)
    save_geotiff(ratio_4_5.astype('float32'), "Ratio_4_5")
    save_geotiff(ratio_5_7.astype('float32'), "Ratio_5_7")
