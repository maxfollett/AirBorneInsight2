...
def filter_high_density_areas(df, gridsize=50, percentile_threshold=25):
    ...
    return df_filtered, hexbin, density, high_density_coords

# Step 3: Generate Hexbin Plot of Kept (High-Density) Data (Updated to Remove Low-Density)
def generate_kept_hexbin_plot(df_high_density, hexbin, density, high_density_coords, gridsize=50):
    # Set the upper and lower limits for the color scale (75th percentile to max density)
    upper_limit = np.percentile(density, 75)
    max_density = np.max(density)
    
    # Plot only hexagons with density above the threshold
    plt.figure(figsize=(8, 6))
    
    # Mask low-density hexagons (below the 25th percentile)
    high_density_hexagons = [coord for i, coord in enumerate(hexbin.get_offsets()) if density[i] >= upper_limit]
    high_density_values = [d for i, d in enumerate(density) if d >= upper_limit]
    
    # Only plot the high-density hexagons
    hb = plt.hexbin(
        [coord[0] for coord in high_density_hexagons],
        [coord[1] for coord in high_density_hexagons],
        C=high_density_values,
        gridsize=gridsize,
        cmap='coolwarm',
        edgecolors='k',
        linewidths=0.3,
    ...
    cb.set_ticks([upper_limit, max_density / 2, max_density])
...
def preprocess_data():
    ...
    df_high_density, hexbin, density, high_density_coords = filter_high_density_areas(df, gridsize=50)
    
    # Generate hexbin plot for the kept (high-density) data (now without low-density points)
    generate_kept_hexbin_plot(df_high_density, hexbin, density, high_density_coords, gridsize=50)
